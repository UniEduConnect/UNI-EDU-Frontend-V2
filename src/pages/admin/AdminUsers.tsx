...
const Avatar = ({
  ...
  src ? (
    ...
    <div
      className={`${className} bg-muted text-foreground/70 flex items-center justify-center text-sm font-semibold`}
    >
      {initial(name)}
    </div>
  );
...
const AdminUsers = () => {
  ...
  const [rejectUserTarget, setRejectUserTarget] =
  ...
  const confirmDelete = () => {
    ...
    deleteUserMut.mutate(deleteTarget.id, {
      onSuccess: () =>
        toast({ title: `Đã xóa ${name}`, variant: "destructive" }),
      onError: (e) =>
        toast({
          title: e instanceof Error ? e.message : "Xóa thất bại",
          variant: "destructive",
        }),
    });
  ...
  const handleCreate = () => {
    if (
      ...
      toast({
        title: "Vui lòng điền email, mật khẩu, họ tên, SĐT",
        variant: "destructive",
      });
      return;
    }
    createUserMut.mutate(addForm, {
      ...
      onError: (e) =>
        toast({
          title: e instanceof Error ? e.message : "Tạo thất bại",
          variant: "destructive",
        }),
    });
  };
  ...
  const handleEdit = () => {
    ...
    updateUserMut.mutate(
      ...
      },
      {
        ...
        onError: (e) =>
          toast({
            title: e instanceof Error ? e.message : "Cập nhật thất bại",
            variant: "destructive",
          }),
      },
    );
  ...
  const handleRejectConfirm = () => {
    ...
    rejectUserMutation.mutate(
      { id: rejectUserTarget.id, payload: { reason } },
      {
        ...
        onError: (e) =>
          toast({
            title: e instanceof Error ? e.message : "Từ chối thất bại",
            variant: "destructive",
          }),
      },
    );
  ...
  const stats = useMemo(
    () => ({
      total: users.length,
      tutors: users.filter((u) => u.role === "tutor" || u.role === "teacher")
        .length,
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        {[
          {
            ...
            value: stats.total,
            icon: Users,
            fg: "from-blue-700 to-blue-900",
          },
          {
            label: "Gia sư & Giáo viên",
            value: stats.tutors,
            icon: GraduationCap,
            fg: "from-blue-700 to-blue-900",
          },
          ...
          <Card
            key={i}
            className={`border-0 shadow-soft text-white bg-gradient-to-r ${s.fg}`}
          >
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10 h-11 rounded-2xl bg-white/80 border border-slate-200 shadow-sm"
          />
        </div>
        <Select
          value={statusFilter}
        ...
        <CardContent className="p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-900">
              Yêu cầu phê duyệt ({pendingApprovals.length})
            </h3>
            <Button
              variant="outline"
              size="sm"
                ...
                pendingApprovals.map((u) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        size="sm"
                        className="rounded-lg h-8 bg-blue-700 text-white hover:bg-blue-800"
                        onClick={() => handleStatusChange(u.id, "approved")}
                      >
                        Duyệt
                      </Button>
                      <Button
                        size="sm"
                        className="rounded-lg h-8 bg-rose-500 text-white hover:bg-rose-600"
                        onClick={() => handleStatusChange(u.id, "rejected")}
                      >
                        Từ chối
                      </Button>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) setRejectUserTarget(null);
        }}
      >
        <DialogContent className="rounded-2xl max-w-md">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Vui lòng cung cấp lý do để cập nhật nhật ký
            </p>
            <Textarea
              value={rejectReason}
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleRejectConfirm} className="h-10">
                Xác nhận
              </Button>
            </div>
          </div>
            ...
            <TableBody>
              {paginated.map((u) => (
                ...
                >
                  ...
                  <TableCell>
                    {u.school ? (
                      <span className="text-xs px-2.5 py-1 rounded-lg bg-muted text-foreground font-medium">
                        {u.school}
                      </span>
                    ) : (
                  ...
                  <TableCell>
                    ...
                    >
                      <SelectTrigger
                        className={`w-28 h-7 text-[11px] rounded-full border-0 font-medium ${statusColor[u.status]}`}
                      >
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  >
                    {isLoading ? (
                      <span className="inline-flex items-center justify-center">
                        <Loader2 className="w-5 h-5 animate-spin mr-2" /> Đang
                        tải người dùng...
                      </span>
                    ) : isError ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="bg-muted/50 p-3 rounded-xl">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[detail.status]}`}
                  >
                ...
                {detail.school && (
                  <div className="bg-muted/50 p-3 rounded-xl col-span-2">
                    ...
                    <span className="text-foreground">{detail.school}</span>
                  </div>
                )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setAddForm((f) => ({ ...f, phoneNumber: e.target.value }))
                  }
                  className="mt-1.5 rounded-xl"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setAddForm((f) => ({ ...f, school: e.target.value }))
                    }
                    className="mt-1.5 rounded-xl"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setAddForm((f) => ({ ...f, degree: e.target.value }))
                    }
                    className="mt-1.5 rounded-xl"
                  />
                </div>
        ...
        <DialogContent className="rounded-2xl max-w-md">
            ...
            <DialogTitle>
              ...
              {editTarget && (
                <span className="text-sm font-normal text-muted-foreground">
                  · {roleLabel[editTarget.role] || editTarget.role}
                </span>
              )}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setEditForm((f) => ({ ...f, degree: e.target.value }))
                      }
                      className="mt-1.5 rounded-xl"
                    />
                  </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button onClick={handleEdit} className="h-10">
                Lưu
              </Button>
            </div>
          </div>
