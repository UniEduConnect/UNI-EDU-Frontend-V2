...
const AdminDashboard = () => {
      ...
      ? Math.round(
          pendingUsers.reduce(
            (sum, u) =>
              sum +
              Math.ceil(
                (now.getTime() - new Date(u.createdAt).getTime()) / 86400000,
              ),
            0,
          ) / pendingUsers.length,
        )
  ...
  const topStats = [
    ...
    },
    {
      ...
      value: `${(monthRevenue / 1000000).toFixed(1)}M`,
      icon: CreditCard,
      change: "+18%",
      up: true,
      bg: "from-blue-700 to-blue-900",
  ...
  const recentClasses = classData
    .filter((c) => c.status === "searching" || c.status === "active")
    .slice(0, 4);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {topStats.map((s) => (
          <Card
            key={s.label}
            className={`border-0 shadow-soft hover:shadow-elevated transition-shadow duration-300 bg-gradient-to-r ${s.bg} text-white`}
          >
          ...
          <CardContent className="p-5">
            ...
            <div className="flex items-end gap-1.5 h-10">
              {[40, 65, 55, 80, 70, 90, 60].map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-sm bg-blue-100/50 relative overflow-hidden"
                  style={{ height: `${h}%` }}
                >
                  <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <Card className="lg:col-span-2 border-0 shadow-soft">
          <CardContent className="p-5">
            <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
              <BarChart3 className="w-4 h-4" /> Doanh thu theo tháng
            </h3>
        ...
        <Card className="border-0 shadow-soft">
          <CardContent className="p-5">
            <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
              <PieChartIcon className="w-4 h-4" /> Cơ cấu người dùng
            </h3>
              ...
              {recentClasses.map((c) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {c.tutorName || "—"} · {c.fee.toLocaleString("vi-VN")}đ
                    </p>
                  </div>
                  <span
                    className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${statusClass[c.status] ?? "bg-slate-100 text-slate-700"}`}
                  >
              ...
              {recentUsers.map((u) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {roleNameMap[u.role] ?? u.role} · {u.email}
                    </p>
                  </div>
                  <span
                    className={`text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0 ${userStatusClass[u.status] ?? "bg-slate-100 text-slate-700"}`}
                  >
