...
const TutorClasses = () => {
  ...
  const { requests: seekingRequests, isLoading: seekingLoading } =
    useOpenClassRequests({ Page: 1 });
  ...
  const tutorTests = useMemo(
    () => exams.filter((e) => e.type === "tutor-test"),
    [exams],
  );
  ...
  const { data: examDetail, isLoading: examDetailLoading } = useExam(
  ...
  const trialDetail = selectedTrial
  ...
  const selectedSubmission =
    viewSubmissionId != null
      ? submissions.find((r) => r.id === viewSubmissionId)
      : null;
  ...
  const filteredClasses = classes.filter((c) => {
    if (
      ...
      return false;
    if (filterStatus !== "all" && statusToBadge[c.status] !== filterStatus)
      return false;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-muted/50 p-1 rounded-xl">
          ...
          <TabsTrigger
            ...
            className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm"
          >
            Đang tìm gia sư ({seekingRequests.length})
          </TabsTrigger>
          <TabsTrigger
            value="trials"
            className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm"
        ...
        <TabsContent value="my-classes" className="mt-4">
          {classesLoading ? (
            <div className="flex items-center justify-center py-16 text-muted-foreground">
              <Loader2 className="w-6 h-6 animate-spin mr-2" /> Đang tải lớp
              học...
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredClasses.map((c: ClassItem) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="bg-card border border-border rounded-2xl p-5 hover:shadow-elevated transition-all group"
                  >
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      ...
                      <button
                        onClick={() => navigate(`/tutor/classes/${c.id}`)}
        ...
        <TabsContent value="seeking" className="mt-4">
          <div className="mb-4 p-3 bg-warning/15 dark:bg-amber-900/10 border border-warning/30 dark:border-warning/40 rounded-xl flex items-center gap-2">
            ...
            <p className="text-xs text-warning dark:text-amber-400">
              Nhận lớp cần làm bài test AI (≥80%) cho đúng môn. Bấm "Nhận lớp"
              để làm test.
            </p>
          </div>
          {seekingLoading ? (
            <div className="flex items-center justify-center py-16 text-muted-foreground">
              <Loader2 className="w-6 h-6 animate-spin mr-2" /> Đang tải yêu
              cầu...
            </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-sm text-muted-foreground">
                Hiện chưa có học sinh nào đang tìm gia sư
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                >
                  ...
                  {r.durationMonths != null && (
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-2">
                      ...
                      <span>Học trong {r.durationMonths} tháng</span>
                    </div>
                  )}
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-3">
                    <CalendarDays className="w-3.5 h-3.5 shrink-0" />
                    <span>Đăng {new Date(r.createdAt).toLocaleDateString("vi-VN")}</span>
                  </div>
                  <button
          ...
          {trialsLoading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {filteredTrials.map((t: TrialItem) => {
                const requestedDate = t.requestedAt
                  ? new Date(t.requestedAt).toLocaleString("vi-VN")
                  : "";
                return (
                  <div
                    key={t.id}
        ...
        <TabsContent value="test-results" className="mt-4 space-y-6">
          ...
          <div>
            <p className="text-sm font-semibold text-foreground mb-3">
              Bài test năng lực khả dụng
            </p>
            {examsLoading ? (
              <div className="flex items-center justify-center py-8 text-muted-foreground">
                <Loader2 className="w-5 h-5 animate-spin mr-2" /> Đang tải bài
                test...
              </div>
            ) : tutorTests.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Hiện chưa có bài test năng lực nào.
              </p>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                {tutorTests.map((e) => (
      ...
      <Dialog
        open={!!selectedTrial}
        onOpenChange={() => setSelectedTrial(null)}
      >
            ...
            <>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4 p-4 bg-muted/30 rounded-xl">
                  ...
                  <div>
                    <p className="text-base font-semibold text-foreground">
                      {trialDetail.studentName}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {trialDetail.subjectName}
                    </p>
                    <span
                      className={cn(
                        "text-[11px] font-medium px-2 py-0.5 rounded-lg mt-1 inline-block",
                        trialStatusColors[trialDetail.status] ??
                          "bg-muted text-muted-foreground",
                      )}
                    >
                      {trialStatusLabels[trialDetail.status] ??
                        trialDetail.status}
                    </span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="p-3 bg-muted/50 rounded-xl">
                    ...
                    <span className="font-medium">
                      {trialDetail.requestedAt
                        ? new Date(trialDetail.requestedAt).toLocaleString(
                            "vi-VN",
                          )
                        : "—"}
                    </span>
      ...
      <Dialog open={!!rejectDialog} onOpenChange={() => setRejectDialog(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Vui lòng nêu lý do từ chối để phụ huynh hiểu.
            </p>
            <textarea
              value={rejectReason}
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center justify-between">
                  <span>
                    Bài kiểm tra{examDetail ? ` - ${examDetail.title}` : ""}
                  </span>
                  {!testResult && (
              ...
              {examDetailLoading ? (
                <div className="flex items-center justify-center py-16 text-muted-foreground">
                  <Loader2 className="w-6 h-6 animate-spin mr-2" /> Đang tải bài
                  test...
                </div>
              ) : !testResult ? (
                <div className="space-y-4">
                      ...
                      <>
                        <div
                          className={cn(
                            "p-8 rounded-2xl",
                            passed
                              ? "bg-success/15 dark:bg-emerald-900/10"
                              : "bg-destructive/5",
                          )}
                        >
                          <p
                          ...
                          <p className="text-xs text-muted-foreground mt-2">
                            ...
                            {testResult.totalQuestions} câu đúng
                          </p>
                        </div>
      ...
      <Dialog
        ...
        onOpenChange={() => setViewSubmissionId(null)}
      >
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-3 text-center text-sm">
                  ...
                  <div className="p-3 bg-muted/50 rounded-xl">
                    ...
                    <p className="font-medium">
                      {selectedSubmission.correctCount}/
                  ...
                  >
                    ...
                    <p className="font-medium">
                      {passedOf(selectedSubmission) ? "Đạt" : "Không đạt"}
                    </p>
                  </div>
                </div>
